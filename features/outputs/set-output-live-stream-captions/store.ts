...
export async function applyOutputLiveStreamCaptions(id: string, captions: boolean): Promise<BroadcastOutputRecord> {
  const [row] = await db
    .update(broadcastOutputs)
    .set({ liveStreamCaptions: captions, updatedAt: sql`now()` })
