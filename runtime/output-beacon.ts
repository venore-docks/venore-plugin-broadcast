...
// viewport, navegador, status e "há quanto tempo no ar". Estado em memória por processo, mesma
// pegada e mesma suposição de processo único do output-bus / pin-attempts (ver README, Known
// Gaps). Não é uma feature (sem handler/service/authorizeActor) — infra de runtime.
//
...
export function recordOutputBeacon(
  token: string,
  clientId: string,
  data: { ip: string; viewport: string; userAgent: string; status: string },
): void {
